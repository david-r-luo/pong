...
display('How Large would you like the left players racket to be? ')
%patch([.05 .07 .07 .05],[.455 .455 .545 .545],[0 0 0 0],'facecolor','w','edgecolor','k',...
    %'tag','left_player')
boardsize = input('1: smaller   2: default   3: larger\n');
while(boardsize ~= 1 && boardsize ~= 2 && boardsize ~= 3)
    boardsize = input('1: smaller   2: default   3: larger\n');
end
if (boardsize == 1)
    patch([.05 .07 .07 .05],[.475 .475 .525 .525], [0 0 0 0],'facecolor','w','edgecolor','k', 'tag','left_player')
elseif (boardsize == 2)
    patch([.05 .07 .07 .05],[.455 .455 .545 .545],[0 0 0 0],'facecolor','w','edgecolor','k', 'tag','left_player')
else 
   patch([.05 .07 .07 .05],[.425 .425 .575 .575],[0 0 0 0],'facecolor','w','edgecolor','k', 'tag','left_player')
end

% Draw right player racket
patch([.95 .93 .93 .95],[.455 .455 .545 .545],[0 0 0 0],'facecolor','w','edgecolor','k',...
    'tag','right_player')

% Draw the ball
h_ball=patch([.492 .508 .508 .492],[.492 .492 .508 .508],[-1 -1 -1 -1],'facecolor','w','edgecolor','w',...
    'tag','ball');

% Add text for score
text(.425 , .86,'0','fontname','courier','fontsize',60,'color','w','hor','center',...
    'tag','score_left')
text(.575 , .86,'0','fontname','courier','fontsize',60,'color','w','hor','center',...
    'tag','score_right');

% Add pause text 
t_pause=text(.5,.5,'Game paused','fontname','courier','fontsize',60,'color','w','hor','center','fontweight','bold',...
    'visible','off');
...
while flag
    ...
    [xdir,ydir,point]=checkpos(x,y,xdir,ydir,sound_level);
...
yd = get(h_right,'ydata'); 
xd = get(h_right, 'xdata');
...
set(h_left,'ydata',cpy)%, 'xdata', cpx)

% End of the WBMFCN function
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [new_xdir,new_ydir,point]=checkpos(x,y,xdir,ydir,sound_level)
